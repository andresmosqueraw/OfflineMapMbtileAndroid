...
// Ejemplo para agregar un punto/marcador
fun addPointToMap(style: Style, lat: Double, lng: Double) {
    val point = Point.fromLngLat(lng, lat)
    
    style.addSource(GeoJsonSource("point-source", point))
    style.addLayer(
        SymbolLayer("point-layer", "point-source")
            .withProperties(
...
// Ejemplo para agregar polígonos personalizados
fun addCustomPolygon(style: Style, coordinates: List<Point>) {
    val polygon = Polygon.fromLngLats(listOf(coordinates))
    
    style.addSource(GeoJsonSource("custom-polygon", polygon))
    style.addLayer(
        FillLayer("custom-polygon-layer", "custom-polygon")
            .withProperties(
